...
export default function CreateGigPage() {
  ...
  const hasVerifiedParentDocuments = (documents: ParentVerificationDocument[]) => {
  ...
  const loadInitialData = async () => {
    try {
      setLoading(true);
      const [documentsData, subjectsData, gradesData] = await Promise.all([
        api.documents.listMyParent(),
        api.profiles.subjects(),
        api.profiles.grades(),
      ]);

      if (!hasVerifiedParentDocuments(documentsData)) {
  ...
  const handleSubmit = async (status: "draft" | "open") => {
    ...
    try {
      ...
      // Convert schedule to the format expected by API
      ...
      const createdGig = await api.gigs.create(gigData);
      ...
      setTimeout(() => {
        navigate(`/parent/gigs/${createdGig.id}`);
      }, 1500);
    } catch (error: any) {
      ...
      if (error.response?.data) {
        ...
        Object.keys(error.response.data).forEach((key) => {
          ...
          const message = Array.isArray(errorValue)
            ...
            : errorValue;
          apiErrors[
            key === "non_field_errors" || key === "detail" ? "general" : key
          ] = message;
        });
        setErrors(apiErrors);
      } else {
          ...
          <Card className="border-yellow-200 bg-yellow-50/60">
            <CardHeader>
              ...
              <CardDescription className="text-yellow-800">
                Your citizenship front and citizenship back must be verified
                before you can create a gig. This prevents you from filling the
                gig form before your account is ready.
              </CardDescription>
            </CardHeader>
            <CardContent className="flex flex-col gap-3 sm:flex-row">
              <Button onClick={() => navigate("/parent/documents")}>
                Go to Documents
              </Button>
      ...
      <div className="container mx-auto p-6 max-w-4xl">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Post a tutoring opportunity and connect with qualified teachers
            </CardDescription>
          </CardHeader>
