...
export default function AdminActiveGigs() {
  ...
  const stats = React.useMemo<ActiveGigStats>(() => {
    const activeGigs = gigs.filter((g) => g.status === "active");

    const avgDuration =
      activeGigs.length > 0
        ? activeGigs.reduce((sum, gig) => sum + gig.duration_weeks, 0) /
          activeGigs.length
        : 0;

    // Gigs completing in next 2 weeks (simplified - would need start date in real app)
    const completingSoon = activeGigs.filter(
  ...
  const loadGigs = async () => {
    try {
      ...
      toast.error("Failed to load active gigs. Please try again.");
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <div className="flex min-h-screen items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-muted-foreground">Loading active gigs...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-linear-to-br from-neutral-50 to-neutral-100">
      <div className="container mx-auto p-6 max-w-7xl">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-3xl font-bold mb-2">Active Gigs</h1>
            <p className="text-muted-foreground">
              Monitor all currently active tutoring sessions
            </p>
          </div>
              ...
              <div className="text-center">
                ...
                <p className="text-muted-foreground mb-6">
                  ...
                  locationFilter !== "all"
                    ...
                    : "There are currently no active gigs"}
                </p>
              </div>
            </CardContent>
          </Card>
      ...
      <Dialog open={showDetailsDialog} onOpenChange={setShowDetailsDialog}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Active gig details and progress information
            </DialogDescription>
          </DialogHeader>
          {selectedGig && (
